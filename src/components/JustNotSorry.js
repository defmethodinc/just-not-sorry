...
const JustNotSorry = ({ phrases, onEvents }) => {
  ...
  const hideWarnings = ({ target }) => {
    if (hasId(target) && warningContainer.current) {
      warningContainer.current.style.visibility = 'hidden';
    } else {
      //no id to cache, clear everything (Microsoft Outlook)
      setWarnings([]);
    }
  };

  const hasId = (target) => target.id !== '';

  const displayWarnings = ({ target }) => {
    const current = warningContainer.current;
    if (hasId(target) && current && current.dataset.emailId === target.id) {
  ...
  const findWarnings = (target) =>
    Util.debounce(() => search(target), Util.WAIT_TIME);
    ...
    while ((textNode = iter.nextNode()) !== null) {
      for (let i = 0; i < phrases.length; i++) {
        ...
        for (let j = 0; j < ranges.length; j++) {
          ...
          let key = `${email.offsetTop + range.startOffset}x${
            email.offsetLeft + range.endOffset
          }`;
          if (range.startContainer.parentElement) {
            ...
            key = `${offsetTop + range.startOffset}x${
              offsetLeft + range.endOffset
            }`;
          }
          warnings.push(
            <Warning
